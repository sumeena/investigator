        ...
        <div class="col-md-10">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form method="post" action="{{ route('company-admin.profile.update') }}">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label" for="basic-default-fullname">First Name</label>
                            <input type="text" class="form-control @error('first_name') is-invalid @enderror"
                                   name="first_name"
                                   value="{{isset($profile) && !empty($profile->first_name) ? $profile->first_name:'' }}">
                            @error('first_name')
                    ...
                    <strong>{{ $message }}</strong>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('last_name') is-invalid @enderror"
                                   name="last_name"
                                   value="{{isset($profile) && !empty($profile->last_name) ? $profile->last_name:'' }}">
                            @error('last_name')
                            <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
                    ...
                    <strong>{{ $message }}</strong>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('phone') is-invalid @enderror" name="phone"
                                   value="{{isset($profile) && !empty($profile->phone) ? $profile->phone:'' }}">
                            @error('phone')
                            <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
                        ...
                        <a href="{{ route('company-admin.reset-password') }}"><button type="button" class="btn btn-primary">Reset Password</button></a>
        </form>
      </div>
    </div>
  </div>
